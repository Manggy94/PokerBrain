...
class Preprocessor:
    ...
    def __init__(self):
        ...
        self.y_transformer.fit([str(combo) for combo in all_combos])
    ...
    def create_vector(self, df: pd.DataFrame, index: int, training: bool = True):
        if training:
    ...
    def create_feature_tab(self, df: pd.DataFrame, index: int, training: bool = True):
        return self.create_vector(df=df, index=index, training=training)[0]

    def create_label_tab(self, df: pd.DataFrame, index: int, training: bool = True):
        return self.create_vector(df=df, index=index, training=training)[1]

    def create_features(self, df: pd.DataFrame, training: bool = True):
        return pd.concat([self.create_feature_tab(df, i, training) for i in range(9)]).reset_index(drop=True)

    def create_labels(self, df: pd.DataFrame, training: bool = True):
        return pd.concat([self.create_label_tab(df, i, training) for i in range(9)]).reset_index(drop=True)

    def create_vectors(self, df: pd.DataFrame, training: bool = True):
        return self.create_features(df, training), self.create_labels(df, training)
